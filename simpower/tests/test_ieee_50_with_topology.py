...
def test_single_period_with_topology():
    """测试单时段案例并生成网络拓扑图"""
...
def test_multi_period_with_topology():
    """测试多时段案例并生成网络拓扑图"""
    
    case_dir = 'simpower/tests/ieee_50_bus_working'
    
    print("\n\n🧪 IEEE 50节点多时段（96时段）案例测试")
    print("=" * 60)
...
def main():
    ...
    print("🚀 IEEE 50节点案例测试（含网络拓扑图绘制）")
    print("=" * 60)
    
    # 1. 测试单时段案例
    single_success = test_single_period_with_topology()
    
    # 2. 测试多时段案例
    multi_success = test_multi_period_with_topology()
    
    # 总结
    print("\n" + "=" * 60)
    print("📊 测试总结:")
    print(f"   单时段案例: {'✅ 成功' if single_success else '❌ 失败'}")
    print(f"   多时段案例: {'✅ 成功' if multi_success else '❌ 失败'}")
    
    if single_success and multi_success:
        print(f"\n🎉 所有测试通过！")
        print(f"📁 结果文件位置:")
        print(f"   单时段: simpower/tests/ieee_50_single_period/results/")
        print(f"   多时段: simpower/tests/ieee_50_bus_working/results/")
        return True
    else:
        print(f"\n⚠️ 部分测试失败")
        return False
